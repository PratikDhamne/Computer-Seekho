...
const MainContent = () => {
  ...
  useEffect(() => {
    const fetchEnquiries = async () => {
      try {
        const response = await fetch(`http://localhost:5051/api/Enquiries`);
        if (!response.ok) {
  ...
  return (
    <main>
      <h2>Welcome, {loggedInUser ? loggedInUser.staffName : 'admin'}!</h2>
      {error ? (
          ...
          <tbody>
            {enquiries.map(enquiry => (
              <tr key={enquiry.enquiryId}>
                ...
                <td>{enquiry.staffId ? enquiry.staffId.staffName : 'N/A'}</td>
              </tr>
            ))}
          </tbody>
        </table>
